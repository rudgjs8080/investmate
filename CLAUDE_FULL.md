...
- **멀티 에이전트 토론** (Bull/Bear/Synthesizer 3라운드) 기본 활성화 + 컨센서스 패널티
- **멀티 호라이즌 피드백** (5d/10d/20d/60d) + 시간 감쇠 가중치 + 점진적 패널티
- **적응형 스코어링** 피드백 상관관계 기반 가중치 자동 최적화 (adaptive 모드)
- **포트폴리오 레벨 AI** 섹터 분포/상관/집중 리스크를 AI 프롬프트에 주입
- **반사실 분석** AI 오버라이드 케이스의 대안 시뮬레이션 → 자기학습
- **시장 체제 감지** (Bull/Bear/Range/Crisis) + 레짐 전환 감지
- **ML 모델** (LightGBM) + 드리프트 감지 + 자동 재학습 트리거
- T+1 실행가 기반 성과 추적, 거래비용 차감
- **팩터 투자 프레임워크** (Value/Momentum/Quality/LowVol/Size z-score) — 레거시/팩터/블렌드/적응형 4모드
- **포지션 사이징** (ERC/Vol Target/Half-Kelly) + 비선형 시그모이드 틸트 + 리밸런싱 트리거
- **실행 현실성** 슬리피지/시장충격 모델(opt-in) + 턴오버 홀드 룰
...
| 웹 대시보드   | FastAPI + Jinja2 + Tailwind     | ECharts 5.x, 16개 라우트, 14개 템플릿                |
| 데이터 처리   | pandas, numpy                   |                                                      |
| 기술적 분석   | ta (Technical Analysis)         |                                                      |
| 스크래핑      | BeautifulSoup4, requests        |                                                      |
| 주식 데이터   | yfinance                        | 배치 다운로드 지원                                   |
| AI 분석       | Anthropic SDK + Claude CLI      | Tool Use 구조화 출력, 4단계 fallback, 강제 규칙 기반 |
| ML            | LightGBM                        | 28피처, 70% 규칙 + 30% ML 블렌딩                     |
| 데이터 검증   | Pydantic v2 + pydantic-settings |                                                      |
| 환경변수      | python-dotenv                   |                                                      |
| 테스트        | pytest, pytest-cov              | 1120개 테스트, 97개 테스트 파일                      |
| PDF 생성      | fpdf2                           | NanumGothic 한글 폰트, 차트 삽입                     |
| 차트          | matplotlib                      | headless Agg 백엔드, 한글 지원                       |
...
│   ├── pipeline.py              # 데일리 파이프라인 오케스트레이터 (STEP 0~6 + 4.5~4.7)
│   ├── weekly_pipeline.py       # 주간 리포트 파이프라인 (5단계: 리포트→AI→PDF→이메일→알림)
│   ├── ai/
│   │   ├── __init__.py
│   │   ├── claude_analyzer.py   # AI 분석 (Tool Use + 4단계 fallback + 스트리밍)
│   │   ├── response_schema.py   # AI 응답 구조화 스키마 (frozen dataclasses)
│   │   ├── validator.py         # AI 결과 검증기 + 신뢰도 캘리브레이션 + enforce_constraints
│   │   ├── feedback.py          # AI 예측 피드백 + ConstraintRules 제약 규칙 자동 생성
...
│   │   ├── scoring_advisor.py   # 적응형 스코어링 가중치 (피드백 상관관계 기반)
│   │   ├── rebalance_trigger.py # AI 리밸런싱 트리거 (손절/레짐/상관 드리프트)
│   │   └── counterfactual.py    # 반사실 분석 (AI 오버라이드 시뮬레이션)
│   ├── db/
│   │   ├── __init__.py
...
│   │   ├── models.py            # ORM 모델 (6 Dimension + 12 Fact + 1 Bridge + 11 Index)
...
│   │   ├── screener.py          # 종목 스크리닝 + 랭킹 엔진 (레거시/팩터/블렌드 모드)
│   │   ├── factors.py           # 팩터 투자 프레임워크 (Value/Momentum/Quality/LowVol/Size z-score)
│   │   ├── factor_returns.py    # 팩터 수익률 추적 (롱숏 스프레드, IC, 팩터 모멘텀)
...
│   │   ├── prompt_builder.py    # AI 프롬프트 생성기 (강제 규칙 기반, 9단계 reasoning)
│   │   ├── comparator.py        # 추천 종목 일간 비교 (신규/이탈/순위변동)
│   │   ├── terminal.py          # Rich 터미널 출력 (두괄식 핵심 요약)
│   │   ├── format_utils.py      # 숫자 포맷 유틸리티 (T/B/M/K 변환)
│   │   ├── weekly_models.py     # 주간 리포트 데이터 모델 (20+ frozen dataclasses)
│   │   ├── weekly_assembler.py  # 주간 리포트 조립기 (14개 섹션 빌더)
│   │   ├── weekly_report.py     # 주간 Markdown/JSON 생성기
│   │   ├── weekly_commentary.py # AI 주간 코멘터리 생성 (Claude SDK)
│   │   ├── weekly_charts.py     # 주간 차트 생성 (matplotlib, VIX/시그널/섹터/매크로)
│   │   ├── weekly_pdf.py        # 주간 PDF 생성 (fpdf2, NanumGothic 한글)
│   │   └── weekly_explainer.py  # 확신 종목 초보자 설명 생성
│   ├── backtest/
│   │   ├── __init__.py
...
│   │   └── drift_detector.py    # 모델 드리프트 감지 (정확도 하락 → 자동 재학습)
│   ├── portfolio/
│   │   ├── __init__.py
│   │   ├── optimizer.py         # 포트폴리오 최적화 (Max Sharpe/Min Var/Risk Parity/Equal)
│   │   ├── efficient_frontier.py # 효율적 프론티어 (Ledoit-Wolf 공분산, 30포인트)
│   │   ├── position_sizer.py    # 포지션 사이징 (ERC/Vol Target/Half-Kelly + 비선형 시그모이드 틸트)
│   │   ├── risk_constraints.py  # 리스크 제약 (종목10%/섹터30%/VaR 2%/레버리지1x)
│   │   ├── drawdown_manager.py  # 드로다운 관리 (ATR 손절 + 포트폴리오 트레일링 스톱)
│   │   ├── execution_cost.py    # 실행 비용 모델 (슬리피지/시장충격/용량 추정)
│   │   ├── turnover.py          # 턴오버 관리 (홀드 룰/히스테리시스 밴드)
│   │   └── return_decomposer.py # 수익률 분해 (gross → spread → impact → fees = net)
│   ├── alerts/
│   │   ├── __init__.py
...
│       │   ├── weekly_report.py # 주간 리포트 목록 + 상세 (2개 라우트)
│       │   └── factors.py      # 팩터 대시보드 + API (/factors, /api/factors/*)
│       ├── templates/           # Jinja2 + Tailwind CSS 템플릿 (14개)
...
│       │   ├── chat.html
│       │   ├── weekly_reports_list.html # 주간 리포트 목록 카드 그리드
│       │   ├── weekly_report.html      # 주간 리포트 상세 (14개 섹션)
│       │   └── factors.html            # 팩터 대시보드 (누적수익률/상관/IC 차트)
│       └── static/
│           ├── charts.js        # ECharts 헬퍼 (스켈레톤 로딩, 토스트, 디바운스)
│           └── style.css        # WCAG AA 접근성, 다크모드 대비
├── tests/                       # 87 테스트 파일, 946개 테스트
│   ├── conftest.py
...
│   ├── test_position_sizer.py   # 포지션 사이징 (ERC/VolTarget/HalfKelly) 테스트
│   ├── test_risk_constraints.py # 리스크 제약 (종목/섹터/VaR/레버리지) 테스트
│   ├── test_drawdown_manager.py # ATR 손절 + 포트폴리오 드로다운 테스트
│   ├── test_pipeline_sizing.py  # STEP 4.6 파이프라인 통합 테스트
│   ├── test_execution_cost.py   # 실행 비용 모델 (슬리피지/임팩트/용량) 테스트
│   ├── test_turnover.py         # 턴오버 계산 + 홀드 룰 테스트
│   ├── test_return_decomposer.py # 수익률 분해 테스트
│   ├── test_backtest_fix.py     # 백테스트 이중차감 수정 검증 테스트
│   ├── test_factors.py          # 팩터 z-score 정규화 + 복합 점수 테스트
│   └── test_factor_returns.py   # 팩터 롱숏 스프레드 + DB 저장/조회 테스트
├── scripts/                     # 자동화 스크립트
...
├── assets/
│   └── fonts/                   # 한글 폰트 (NanumGothic-Regular/Bold.ttf)
├── reports/                     # 생성된 리포트 (구조화)
│   ├── daily/                   # 일별 리포트 (MD + JSON)
│   ├── weekly/                  # 주간 리포트 (MD + JSON + PDF + AI 코멘터리)
│   ├── prompts/                 # AI 분석용 프롬프트
...
class DailyPipeline:
    def run(self):
        ...
        self.step4_screen()             # 스크리닝 + 랭킹 (팩터 모드 지원) + 뉴스 수집
        self.step4_5_ai_analysis()      # Claude AI 분석 (Tool Use + 4단계 fallback)
        self.step4_6_position_sizing()  # 포지션 사이징 + 리스크 제약 + 턴오버 + 실행 비용
        self.step4_7_factor_returns()   # 팩터 롱숏 수익률 계산 + DB 저장
        self.step5_report()             # 두괄식 데일리 리포트 생성 (비중/비용 포함)
...
**팩터 투자 프레임워크 (factors.py + factor_returns.py):**

- **3가지 스코어링 모드** (`INVESTMATE_FACTOR_MODE`):
  - `legacy` (기본값): 기존 5차원 스코어링 — 완전 호환
  - `factor`: 학술적 팩터 프레임워크만 사용 (Value/Momentum/Quality/LowVol/Size)
  - `blend`: 레거시 × (1-ratio) + 팩터 × ratio 블렌딩 (기본 ratio=0.5)
- **11개 서브팩터** (cross-sectional z-score 정규화):
  - Value: Earnings Yield, Book-to-Price, FCF Yield
  - Momentum: 12-1개월 수익률, 52주 고가 비율
  - Quality: ROE 안정성, Gross Profit/Assets, 발생주의
  - Low Volatility: 60일 실현 변동성, 하방 편차
  - Size: ln(시가총액)
- **레짐별 팩터 가중치**: bull=모멘텀↑, bear=가치+품질↑, crisis=품질+저변동↑
- **팩터 수익률**: 일별 롱숏 스프레드 (상위 20% - 하위 20%) → fact_factor_returns 저장
- **레거시 보존**: 기존 `_score_*` 함수 5개는 `_legacy_score()`로 래핑, 일체 수정 없음
...
**프롬프트 시스템 (prompt_builder.py — 강제 규칙 기반):**

- **페르소나**: "리스크 매니저 겸 퀀트 애널리스트" (손실 회피 최우선)
- **`<hard_rules>` 6개 강제 규칙**: 신뢰도 상한(VIX 기반), 추천 수 제한, 섹터 차단, 캘리브레이션, 목표가 보수적, 피드백 강제
- **`<reasoning_process>` 9단계 사고**: 체제 판단→스타일→차단 확인→분석→신뢰도→캘리브레이션→상한→추천수→목표가
- **`<blocked_sectors>`**: 승률 40% 미만 섹터 자동 차단
- **`<calibration>`**: 과거 신뢰도별 실제 승률 테이블 삽입
- **`<feedback_rules>`**: 명령형 피드백 ("~하세요") 자동 생성
- **자동 스타일 전환**: VIX/승률 기반 방어적/균형 자동 결정
- **ThreadPoolExecutor**: 프롬프트 데이터 병렬 조립 (보강 데이터, 이벤트, 피드백)
- **ConstraintRules**: feedback.py에서 시장 체제/성과 기반 제약 자동 생성

**AI 검증 및 피드백 (이중 방어):**

- **validator.py**: 기본 검증(목표가/손절가 보정) + `enforce_constraints()` 강제 적용
  - 신뢰도 상한 절삭 (VIX 기반)
  - 차단 섹터 강제 제외
  - 추천 수 초과 시 하위 신뢰도부터 제거
  - 경고 3건 이상 → 프롬프트 조정 필요 신호
- **feedback.py**: ECE, 교정 곡선, 섹터별 승률 + `generate_constraint_rules()` 제약 생성
...
### STEP 4.6 -- 포지션 사이징 + 리스크 관리

**position_sizer.py — 3가지 전략:**

| 전략       | 설명                                         |
| ---------- | -------------------------------------------- |
| Vol Target | 목표 변동성(15%)에 맞춰 노출도 조절 (기본값) |
| ERC        | Equal Risk Contribution, 한계 리스크 균등화  |
| Half-Kelly | Kelly Criterion의 절반, 기대수익률 기반      |

- AI 신뢰도 틸트: `ai_confidence / 5.0` 비율로 비중 조정 (5=중립)

**risk_constraints.py — 하드/소프트 제약:**

| 제약           | 종류   | 기본값            |
| -------------- | ------ | ----------------- |
| 단일 종목 상한 | 하드   | 10%               |
| 단일 섹터 상한 | 하드   | 30%               |
| 일일 VaR 한도  | 하드   | 2% (95% 신뢰구간) |
| 레버리지 상한  | 하드   | 1.0x              |
| 평균 상관관계  | 소프트 | 0.5 초과 시 경고  |

**drawdown_manager.py — 드로다운 관리:**

- 포트폴리오 트레일링 스톱: 고점 대비 -10% → 노출도 50% 축소
- 개별 종목 손절: AI stop_loss 우선 (3-15% 범위), 없으면 ATR×2 fallback

**turnover.py — 턴오버 관리:**

- 일일 턴오버 = Σ|w_new - w_old| / 2, 연환산 1200% 초과 시 경고
- 홀드 룰: 손절 미트리거 + 점수 하위 30% 아닌 종목은 유지
- 히스테리시스 밴드: 매수 임계(1%) > 매도 임계(0.5%)

**execution_cost.py — 실행 비용 모델 (opt-in):**

- 슬리피지: 스프레드/2 (S&P 500 ~2bps)
- 시장충격: σ_daily × sqrt(participation_rate) (Almgren-Chriss 단순화)
- 용량 제약: ADTV의 1% 이하 참여, 전략 최대 AUM 추정
- 기존 `INVESTMATE_TX_COST_BPS` 고정 비용과 공존

**DB 컬럼 추가 (fact_daily_recommendations):**

- position_weight, trailing_stop, atr_stop, sizing_strategy
- spread_cost_bps, impact_cost_bps, total_cost_bps, daily_turnover

### STEP 4.7 -- 팩터 수익률 계산

- 5개 팩터 카테고리별 롱숏 스프레드 (상위 20% - 하위 20% 일간 수익률)
- `fact_factor_returns` 테이블에 저장
- 팩터 모멘텀: 1/3/6개월 누적 스프레드로 유효 팩터 판별
...
# 일요일 10:00 KST — 주간 리포트 (AI 코멘터리 + PDF + 이메일 발송)
0 10 * * 0 cd /home/ec2-user/investmate && source .venv/bin/activate && investmate report weekly >> /home/ec2-user/investmate/logs/weekly.log 2>&1
...
| `/weekly-reports`          | 주간 리포트 목록 (카드 그리드, KPI 배지)    |
| `/weekly-report/{week_id}` | 주간 리포트 상세 (14개 섹션, 차트)          |
| `/factors`                 | 팩터 대시보드 (누적수익률, IC, 상관관계)    |
...
- 포지션 사이징: **position_weight**, **trailing_stop**, **atr_stop**, **sizing_strategy**
- 실행 비용: **spread_cost_bps**, **impact_cost_bps**, **total_cost_bps**, **daily_turnover**
...
#### fact_factor_returns (팩터 롱숏 수익률)

- date_id, factor_name, long_return, short_return, spread
- **인덱스:** `idx_factor_returns_date`, UNIQUE(date_id, factor_name)
...
**포지션 사이징 (position_sizer.py) — 3가지 전략:**

| 전략       | 설명                                           |
| ---------- | ---------------------------------------------- |
| Vol Target | 목표 연환산 변동성에 맞춰 노출도 조절 (기본값) |
| ERC        | Equal Risk Contribution, 한계 리스크 균등화    |
| Half-Kelly | Kelly Criterion 절반, 기대수익률 기반          |

- **AI 신뢰도 틸트**: confidence/5.0 비율로 비중 조정 후 정규화
- **파이프라인 통합**: STEP 4.6에서 자동 실행, fact_daily_recommendations에 position_weight 기록

**리스크 제약 (risk_constraints.py):**

- 하드 리밋: 종목 10%, 섹터 30%, VaR 2%, 레버리지 1.0x
- 소프트 리밋: 상관관계 집중도, 팩터 쏠림 경고
- 위반 시 초과 비중을 현금으로 환원 (연쇄 위반 방지)

**드로다운 관리 (drawdown_manager.py):**

- 포트폴리오 트레일링 스톱: 고점 대비 -10% → 50% 축소
- 개별 종목: AI stop_loss 우선 → ATR×2 fallback

**실행 비용 모델 (execution_cost.py, opt-in):**

- 슬리피지 + 시장충격(Almgren-Chriss) + 용량 제약(ADTV 1%)
- 전략 최대 AUM 추정, 기존 고정 20bps와 공존

**턴오버 관리 (turnover.py):**

- 홀드 룰 + 히스테리시스 밴드로 불필요한 회전 방지
- 연환산 1200% 초과 시 경고
...
| INVESTMATE_SIZING_STRATEGY    | vol_target                | 사이징 전략 (erc/vol_target/half_kelly) |
| INVESTMATE_TARGET_VOL         | 15.0                      | 목표 변동성 (연간 %)                    |
| INVESTMATE_MAX_STOCK_PCT      | 0.10                      | 단일 종목 최대 비중                     |
| INVESTMATE_MAX_SECTOR_WEIGHT  | 0.30                      | 단일 섹터 최대 비중                     |
| INVESTMATE_DAILY_VAR_LIMIT    | 2.0                       | 일일 VaR 한도 (%)                       |
| INVESTMATE_TRAILING_STOP      | 10.0                      | 트레일링 스톱 (%)                       |
| INVESTMATE_ATR_MULTIPLIER     | 2.0                       | ATR 손절 배수                           |
| INVESTMATE_EXEC_COST_ENABLED  | false                     | 실행 비용 모델 활성화                   |
| INVESTMATE_SPREAD_BPS         | 2.0                       | 스프레드/2 (bps)                        |
| INVESTMATE_IMPACT_COEFF       | 1.0                       | 시장충격 계수                           |
| INVESTMATE_MAX_PARTICIPATION  | 0.01                      | ADTV 최대 참여율                        |
| INVESTMATE_TURNOVER_WARN      | 12.0                      | 턴오버 경고 임계값 (연환산)             |
| INVESTMATE_FACTOR_MODE        | legacy                    | 팩터 모드 (legacy/factor/blend)         |
| INVESTMATE_FACTOR_BLEND_RATIO | 0.5                       | 팩터 블렌딩 비율                        |
| INVESTMATE_FEEDBACK_HORIZONS  | 5,10,20,60                | 멀티 호라이즌 피드백 수평선 (일)        |
| INVESTMATE_FEEDBACK_DECAY_HALFLIFE | 30                   | 피드백 시간 감쇠 반감기 (일)            |
| INVESTMATE_DEBATE_CONSENSUS_PENALTY | 1                   | 낮은 합의 시 신뢰도 차감 점수           |
| INVESTMATE_PORTFOLIO_CONTEXT_ENABLED | True               | 포트폴리오 컨텍스트 AI 주입 활성화      |
| INVESTMATE_SIZING_TILT_MODE   | sigmoid                   | 신뢰도 틸트 모드 (linear/sigmoid/calibrated) |
| INVESTMATE_REBALANCE_ENABLED  | True                      | AI 리밸런싱 트리거 활성화               |
| INVESTMATE_COUNTERFACTUAL_ENABLED | True                  | 반사실 분석 활성화                      |
| INVESTMATE_ML_DRIFT_THRESHOLD | 0.10                      | ML 드리프트 감지 임계값 (정확도 하락)   |
...
investmate report weekly        # 주간 리포트 생성 (AI+PDF+이메일)
investmate report weekly-latest # 최근 주간 리포트 출력
...
- **테스트:** in-memory SQLite, Mock 외부 API, 946개 테스트 (87개 파일)
