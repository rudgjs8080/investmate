...
| 데이터 | yfinance, pandas, ta | 배치 다운로드, 프로바이더 추상화, 서킷브레이커, 데이터 검증 |
...
- **STEP 1**: yfinance 배치 수집 (증분, 실패 티커 추적, 서킷브레이커 보호), 재무(ThreadPool max_workers=4), 매크로(VIX/금리/달러/금/유가 배치 1회), 강화(내부자/기관/애널리스트/실적/공매도), 뉴스(v2 API)
